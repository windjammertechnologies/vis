...
class LayoutEngine {
    ...
    if (this.options.hierarchical.enabled === true && this.body.nodeIndices.length > 0) {
      ...
      if (undefinedLevel === true && definedLevel === true) {
        ...
        for (let nodeId in this.body.nodes) {
          if (this.body.nodes.hasOwnProperty(nodeId)) {
            node = this.body.nodes[nodeId];
            if (!levelSizes[node.options.level]) levelSizes[node.options.level] = 0;
            levelSizes[node.options.level] = Math.max(levelSizes[node.options.level], isVertical ? node.shape.height : node.shape.width);
          }
        }
        this.hierarchical.levelSize = {};
        for (let nodeId in this.body.nodes) {
          if (this.body.nodes.hasOwnProperty(nodeId)) {
            node = this.body.nodes[nodeId];
            this.hierarchical.levelSize[nodeId] = levelSizes[node.options.level] || 0;
          }
        }
  ...
  /**
   * This function places the nodes on the canvas based on the hierarchical distribution.
   *
  ...
  _getDistribution() {
    ...
    for (nodeId in this.body.nodes) {
      if (this.body.nodes.hasOwnProperty(nodeId)) {
        ...
        let levelSize = this.hierarchical.levelSize[nodeId] === undefined? 0 : this.hierarchical.levelSize[nodeId];
        this.direction.fix(node, level, levelSize);
