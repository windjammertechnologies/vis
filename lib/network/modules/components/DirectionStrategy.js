...
class DirectionInterface {
   ...
   * @param {number} level The level size as an offset added to each level
   */
  fix(node, level, levelSize) { this.fake_use(node, level, levelSize); this.abstract(); }
...
class VerticalStrategy extends DirectionInterface {
  ...
  /** @inheritdoc */
  fix(node, level, levelSize) {
    node.y = this.layout.options.hierarchical.levelSeparation * level + levelSize;
...
class HorizontalStrategy extends DirectionInterface {
  ...
  /** @inheritdoc */
  fix(node, level, levelSize) {
    node.x = this.layout.options.hierarchical.levelSeparation * level + levelSize;
